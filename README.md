...
transfer() {
    ...
    # Collect files first so an invalid invocation does not create an empty
    # remote directory. Directory uploads preserve the top directory name.
    local items=()
    ...
    # export -f, which zsh does not support). Each path component is URL
    # encoded so spaces and shell metacharacters upload correctly.
